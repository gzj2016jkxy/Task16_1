				...
				<td valign="top">
					<div align="center">
							...
							<tbody>
								<tr>
									<td colspan="4"><font color="#ff0000"><font
											color="#333399">商品分类：</font></font>【<a href="index.jsp">鲜花</a>】 【<%=goodsPrice%>】</td>
								</tr>
								<tr>
						...
						<p class="style5">
							页次：<%=currentPage%>/<%=pageCount%>
							页 共有<%=rowCount%>种商品 <a
								href="getGoodsByPrice.action?searchBy=<%=goodsPrice%>&currentPage=1">首页
							</a>-
							<%
							if (currentPage == 1) {
						%>
						...
						%>
							<a
								href="getGoodsByPrice.action?searchBy=<%=goodsPrice%>&currentPage=<%=currentPage - 1%>">上一页
								-</a>
							<%
								}
							%>
							...
							%>
							<a
								href="getGoodsByPrice.action?searchBy=<%=goodsPrice%>&currentPage=<%=currentPage + 1%>">下一页-
							</a>
							<%
								}
							%>
							<a
								href="getGoodsByPrice.action?searchBy=<%=goodsPrice%>&currentPage=<%=pageCount%>">末页
							</a>
							<%
								for (int j = 0; j < pageCount; j++) {
							%>
							<a
								href="getGoodsByPrice.action?searchBy=<%=goodsPrice%>&currentPage=<%=j+1%>">[<%=j + 1%>]
							</a>
							<%
								}
							%>
