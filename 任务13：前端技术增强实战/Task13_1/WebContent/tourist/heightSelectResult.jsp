...
<%
	...
	String goodsPrice=(String)session.getAttribute("goodsPrice");
				...
				<td bgcolor="#ffffff" colspan="7">
					<div align="right">
						<a name="top"></a><a href="../register/cart.jsp"><img
							...
							border="0"></a> <a onfocus="this.blur()" href="../register/checkMyInfo.jsp"><img
				...
				<td valign="top">
					<div align="center">
						...
						<table cellspacing="0" cellpadding="0" width="19%">
							<tbody>
								...
								<tr>
									...
									<td>
										<div align="center">
											<a href='<%=path%>/getGood.action?goodsId=<%=temp.getGoodsId()%>' target=_self>
												<img height=100
												<%str = "image/" + temp.getGoodsId() + ".jpg";%>
												src=<%=str%> width=100 border=1>
											</a> <a href='<%=path%>/getGood.action?goodsId=<%=temp.getGoodsId()%>' target=_self>
												<br /> <%=temp.getGoodsName()%>
											</a> <br /> 北京市区价：<font color=#ff6600><br><%=temp.getGoodsPrice()%>元<br></font>
											<a href='<%=path%>/getGood.action?goodsId=<%=temp.getGoodsId()%>' target=_self>
												<img height=17 src="image/detail.gif" width=40
												border=0>
												<br/>
												<br/>
											</a>
						...
						<p class="style5">
							页次：<%=currentPage%>/<%=pageCount%>
							页 共有<%=rowCount%>种商品 <a
								href="getGoodsByHeightSelect.action?searchBy=<%=searchBy%>&searchOption=<%=searchOption%>&goodsPrice=<%=goodsPrice%>&sortName=<%=sortName%>&currentPage=1">首页
							</a>-
							<%
							if (currentPage == 1) {
						%>
						...
						%>
							<a
								href="getGoodsByHeightSelect.action?searchBy=<%=searchBy%>&searchOption=<%=searchOption%>&goodsPrice=<%=goodsPrice%>&sortName=<%=sortName%>&currentPage=<%=currentPage - 1%>">上一页
								-</a>
							<%
								}
							%>
							...
							%>
							<a
								href="getGoodsByHeightSelect.action?searchBy=<%=searchBy%>&searchOption=<%=searchOption%>&goodsPrice=<%=goodsPrice%>&sortName=<%=sortName%>&currentPage=<%=currentPage + 1%>">下一页-
							</a>
							<%
								}
							%>
							<a
								href="getGoodsByHeightSelect.action?searchBy=<%=searchBy%>&searchOption=<%=searchOption%>&goodsPrice=<%=goodsPrice%>&sortName=<%=sortName%>&currentPage=<%=pageCount%>">末页
							</a>
							<%
								for (int j = 0; j < pageCount; j++) {
							%>
							<a
								href="getGoodsByHeightSelect.action?searchBy=<%=searchBy%>&searchOption=<%=searchOption%>&goodsPrice=<%=goodsPrice%>&sortName=<%=sortName%>&currentPage=<%=j+1%>">[<%=j + 1%>]
							</a>
							<%
								}
							%>
