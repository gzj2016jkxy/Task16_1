				...
				<td valign="top">
					<div align="center">
							...
							<tbody>
								<tr>
									<td colspan="4"><font color="#ff0000"><font
											color="#333399">商品分类：</font></font>【<a href="index.jsp">鲜花</a>】 
											【关键字：<%=goodsName%>&nbsp;查询分类：
											<%
												if(sortName.equals("%")){
													out.print("所有分类");
						...
						<p class="style5">
							页次：<%=currentPage%>/<%=pageCount%>
							页 共有<%=rowCount%>种商品 <a
								href="getGoodsByLowerSelect.action?goodsName=<%=goodsName%>&sortName=<%=sortName%>&currentPage=1">首页
							</a>-
							<%
							if (currentPage == 1) {
						%>
						...
						%>
							<a
								href="getGoodsByLowerSelect.action?goodsName=<%=goodsName%>&sortName=<%=sortName%>&currentPage=<%=currentPage - 1%>">上一页
								-</a>
							<%
								}
							%>
							...
							%>
							<a
								href="getGoodsByLowerSelect.action?goodsName=<%=goodsName%>&sortName=<%=sortName%>&currentPage=<%=currentPage + 1%>">下一页-
							</a>
							<%
								}
							%>
							<a
								href="getGoodsByLowerSelect.action?goodsName=<%=goodsName%>&sortName=<%=sortName%>&currentPage=<%=pageCount%>">末页
							</a>
							<%
								for (int j = 0; j < pageCount; j++) {
							%>
							<a
								href="getGoodsByLowerSelect.action?goodsName=<%=goodsName%>&sortName=<%=sortName%>&currentPage=<%=j+1%>">[<%=j + 1%>]
							</a>
							<%
								}
							%>
