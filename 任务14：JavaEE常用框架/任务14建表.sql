...
CREATE TABLE `tab_artile` (
	...
	`Aclass` int(11) NOT NULL ,						--所属类别ID
	`Atitle` varchar(50) NOT NULL,					--文章标题
	`Acontent` varchar(5000) default NULL,			--文章内容
	`Adate` varchar(40) NOT NULL,					--添加时间	
	PRIMARY KEY (`id`)								--主键
	) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT = '博文信息表';

------------------------------------------------------------
--博文类别信息表tab_artitle_class
------------------------------------------------------------
DROP TABLE IF EXISTS `tab_artitle_class`;	
CREATE TABLE `tab_artitle_class` (
	`id` int(11) NOT NULL auto_increment,			--自动编号
	`Acname` varchar(50) NOT NULL,					--文章分类名称	
	`Acdate` varchar(40) NOT NULL,					--添加时间	
	PRIMARY KEY (`id`)								--主键
	) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT = '博文类别信息表';

------------------------------------------------------------
--博文评论信息表tab_article_commend
------------------------------------------------------------
DROP TABLE IF EXISTS `tab_article_commend`;	
CREATE TABLE `tab_article_commend` (
	`id` int(11) NOT NULL auto_increment,			--自动编号
	`Cid` int(11) NOT NULL ,						--文章 ID 编号
	`Cname` varchar(50) default '游客',				--昵称
	`Ccontent` varchar(200) default NULL,			--评论内容
	`Cdate` varchar(40) NOT NULL,					--评论时间	
	PRIMARY KEY (`id`)								--主键
...
CREATE TABLE `tab_manager` (
	`id` int(11) NOT NULL auto_increment,			--自动编号
	`Mname` varchar(50) NOT NULL,					--管理员名称
	`Mpasswd` varchar(50) NOT NULL,					--密码
	PRIMARY KEY (`id`)								--主键
...
CREATE TABLE `tab_photo` (
	`id` int(11) NOT NULL auto_increment,			--自动编号
	`Pclass` int(11) NOT NULL ,						--相册分类ID
	`Pname` varchar(50) NOT NULL,					--图片名称
	`Ppic` varchar(50) default NULL,				--图片标识
	`Pinfo` varchar(100) default NULL,				--图片信息
	`Pdate` varchar(40) NOT NULL,					--添加时间	
	PRIMARY KEY (`id`)								--主键
	) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT = '照片信息表';

------------------------------------------------------------
--照片类别信息表tab_photo_class
------------------------------------------------------------
DROP TABLE IF EXISTS `tab_photo_class`;	
CREATE TABLE `tab_photo_class` (
	`id` int(11) NOT NULL auto_increment,			--自动编号
	`Pcname` varchar(50) NOT NULL,					--相册分类名称
	`Pcdate` varchar(40) NOT NULL,					--添加时间	
	PRIMARY KEY (`id`)								--主键
	) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT = '照片类别信息表';

------------------------------------------------------------
--管理员信息表tab_manager插入一条记录
------------------------------------------------------------	
insert into tab_manager values(1,'admin','admin');

------------------------------------------------------------
--博文类别信息表tab_artitle_class插入记录
------------------------------------------------------------	
...
------------------------------------------------------------
--博文评论信息表tab_article_commend插入记录
------------------------------------------------------------	
